...
## Corpus Check
- 62 files · ~40,282 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 577 nodes · 1125 edges · 21 communities (20 shown, 1 thin omitted)
...
Cohesion: 0.27
Nodes (8): ndarray, Block 5a: research-side trading cost model (DESIGN.md Block 5's cost components,, Per-security $ trading cost for one rebalance's ``delta_w = w - w_prev``.      `, trade_cost(), Square-root impact: the cost RATE (bps) grows as sqrt(size), not the     total $, test_impact_rate_scales_sublinearly_with_trade_size(), test_lower_adv_gives_higher_cost_for_same_trade(), test_zero_trade_gives_zero_cost()
...
- **Why does `PITStore` connect `test_crsp_loader.py` to `test_store.py`, `model.py`, `YFinanceDailyLoader`, `.load_year`, `test_security_master.py`, `test_signals.py`, `test_ic.py`, `build_exposure_matrix`, `build_optimizer_inputs`, `build_exposure_matrix`?**
  _High betweenness centrality (0.412) - this node is a cross-community bridge._
- **Why does `build_risk_model()` connect `model.py` to `build_optimizer_inputs`, `build_exposure_matrix`, `test_crsp_loader.py`?**
