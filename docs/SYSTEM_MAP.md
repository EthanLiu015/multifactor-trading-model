...
### engine/ — **built (broker simulator + order gateway + position keeper + risk checker + market data handler, 38 Catch2 tests)**
...
| `CMakeLists.txt` (root + tests/) | Catch2 pinned via FetchContent (v3.9.1); IXWebSocket v12.0.1 (`USE_TLS=ON`, auto-selects Apple SecureTransport on macOS) + nlohmann/json v3.12.0 also via FetchContent; `build/` gitignored; `broker_sim` library now also builds `src/order/OrderGateway.cpp` + `src/position/PositionKeeper.cpp` + `src/risk/RiskChecker.cpp` + `src/marketdata/MarketDataHandler.cpp` + `src/marketdata/AlpacaMarketDataStream.cpp`; links `nlohmann_json::nlohmann_json` (PRIVATE, .cpp-only use) and `ixwebsocket::ixwebsocket` (PUBLIC -- `AlpacaMarketDataStream.hpp` holds an `ix::WebSocket` member by value, so it leaks into the public interface) |
...
### tests/ — **106 passing, 1 skipped (Python)** + **38 passing (C++, `ctest --test-dir engine/build`)**
| file | covers |
|---|---|
...
security master block 4 (CRSP permno/CUSIP hookup) · `research/attribution/` · `engine/` remaining Block 5 (C++) piece: live `AlpacaGateway` (order gateway state machine + position keeper + pre-trade risk checks + market data handler now built; separate from the now-complete research/backtest/) · `common/` · `infra/`
