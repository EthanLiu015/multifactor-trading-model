...
### research/attribution/ — **Block 6 Attribution + book scorecard v1 BUILT (2026-08-05, 10 tests)**
...
| `scorecard.py` — `BookScorecard` (dataclass), `score_backtest(result, steps)` → `BookScorecard` | book-level metrics (DESIGN.md's analytics suite): Sharpe ratio (annualized, `mean/std*sqrt(252)`, `None` if <2 points or zero variance), max drawdown (peak-to-trough on `BacktestResult.equity_curve`), hit rate (fraction of periods with `net_return > 0`), gross/net exposure per date (`Σ\|w_i\|` / `Σw_i` from each `BacktestStep`'s weights). Takes both `result` (`BacktestResult`, the scalar series) and the original `steps` (need each step's weight vector, which `BacktestResult` itself never carries) -- filters `steps` to `period_return is not None` internally, matching `summarize_backtest`'s own exact filter (verified by reading its source, not assumed "drop the last one") so the exposure arrays stay aligned with `result`'s arrays. Deliberately NOT built (documented gaps): per-factor paper-portfolio metrics (needs a hypothetical single-factor-only portfolio, not just `decompose_backtest`'s aggregate contribution), realized transfer coefficient (meaningless until the backtester models a real target-vs-held execution gap -- current single-shot fill assumption makes held≡target), realized beta (needs `research/portfolio/beta.py` wired in). IC time series + t-stats already exist separately (`research/alpha/ic.py`, Phase 2), not yet combined into this scorecard. |
...
### tests/ — **119 passing, 1 skipped (Python)** + **66 passing (C++, `ctest --test-dir engine/build`)**
| file | covers |
|---|---|
...
| `test_scorecard.py` | Sharpe ratio matches hand computation / `None` under 2 points / `None` at zero variance, max drawdown matches a hand-computed peak-to-trough example, hit rate matches hand computation, gross/net exposure stay aligned with `result`'s arrays when a step is excluded (mid-list, not just the last one) |
