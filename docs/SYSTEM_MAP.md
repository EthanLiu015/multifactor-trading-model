...
| `model.py` — `RiskModel` (dataclass, `.sigma()`), `build_factor_return_history`, `build_risk_model` | orchestrates all of the above into Σ = B·F·Bᵀ + diag(D) for one date, using trailing history for F/D. `RiskModel.sigma()`'s docstring documents the benign Apple Accelerate BLAS warning quirk found here. `build_factor_return_history` widened 2026-08-05 (block 6 attribution) to also return `exposure_history` — the per-date exposure matrices it already builds internally for the regression, previously discarded once F/D were estimated; `build_risk_model` unpacks and ignores its own copy (`_exposure_history`), `research/attribution/decompose.py` is the real consumer |

### research/portfolio/ — **Block 4 Optimizer BUILT (kicked off 2026-07-20, 17 tests)**
...
| `export_target.py` — `export_target_portfolio(target, book_notional=10_000_000.0, path=common/target_portfolio.json)` → `Path` | writes the file contract the C++ `ExecutionScheduler` reads (DESIGN.md's `common/` "target portfolio file/IPC format", scoped+created 2026-08-04) -- target DOLLAR NOTIONAL per symbol (`weight * book_notional`), not shares or weights, since execution converts to shares from its own live price. Zero-weight symbols skipped. `security_ids` are already ticker strings (yfinance's native `security_id` -- confirmed by reading `research/risk/exposures.py`'s membership join, NOT `security_master`'s `internal_id`, a separate unintegrated identifier system), so no resolution step is needed. `status` passed through as-is; the reader, not this writer, decides whether to trade on it. See `common/README.md` for the full schema. |
...
### research/attribution/ — **Block 6 Attribution + book scorecard + capacity analysis v1 BUILT (2026-08-06, 14 tests)**
New directory, DESIGN.md's own reserved name for this block. First piece of Block 6 Performance Analysis + the analytics suite — decomposes realized backtest returns, runs on backtest output first (DESIGN.md: "same metric code runs on backtest output and live data," backtest is what exists right now).

| file / symbol | does |
|---|---|
| `decompose.py` — `AttributionResult` (dataclass), `decompose_backtest(steps, store, knowledge_ts=None)` → `AttributionResult` | per rebalance date: how much of that date's target weights' realized return came from factor bets (`Σ_k portfolio_exposure_k × factor_return_k`) vs specific/idiosyncratic return (`Σ_i weight_i × specific_return_i`) — DESIGN.md Block 6's "how much P&L came from intended factor bets vs specific returns." Reuses the SAME per-date exposures/factor-returns/specific-returns `build_risk_model` already computes internally (via `research/risk/model.py`'s `build_factor_return_history`, widened for this) rather than running a second independent regression. Fully vectorized (Polars `unpivot`+join+`group_by`) — no new Python loop over dates; `build_factor_return_history` already contains the sole per-date loop this needs. Steps with `status != "optimal"` excluded (weights aren't real trades, same convention as `build_target_portfolio`); a date with no matching factor/specific data contributes `0.0`/`0.0`, not a gap — every step's date is represented in the output. **Worth noting, not fixed here**: `build_factor_return_history` itself already contains a loop over dates, predating coding-conventions §8's "backtester's day loop is the sole sanctioned exception" — a pre-existing inconsistency in the house rule, not introduced or resolved by this block. |
| `scorecard.py` — `BookScorecard` (dataclass), `score_backtest(result, steps, store, knowledge_ts=None)` → `BookScorecard` | book-level metrics (DESIGN.md's analytics suite): Sharpe ratio (annualized, `mean/std*sqrt(252)`, `None` if <2 points or zero variance), max drawdown (peak-to-trough on `BacktestResult.equity_curve`), hit rate (fraction of periods with `net_return > 0`), gross/net exposure per date (`Σ\|w_i\|` / `Σw_i` from each `BacktestStep`'s weights), realized beta per date (`research/portfolio/beta.py`'s `compute_market_beta` dotted with weights -- DESIGN.md's "realized beta (should sit ~0 — the beta-neutrality check)"; `NaN` where beta is unavailable, e.g. insufficient trailing history). Takes both `result` (`BacktestResult`, the scalar series) and the original `steps` (need each step's weight vector, which `BacktestResult` itself never carries) -- filters `steps` to `period_return is not None` internally, matching `summarize_backtest`'s own exact filter (verified by reading its source, not assumed "drop the last one") so every per-step array stays aligned with `result`'s arrays. Realized beta is computed via a loop over `steps` -- a deliberate, flagged exception to coding-conventions §8 (a third instance alongside the backtester's own loop and `research/risk/model.py`'s pre-existing one): `compute_market_beta` has no vectorized-across-dates form, and the loop runs over an already-completed backtest's rebalance dates (small N), not a big-data table (Ethan's call, 2026-08-05). Real bug found+fixed getting the tests working: `store.asof()` on a dataset that was never appended to raises Polars' `ComputeError` (scan on an empty glob), not an empty result (handoff.md §13's documented quirk) -- `score_backtest` always reads `BARS_DATASET`, so every test needs that directory to exist even if it doesn't care about beta. Still deliberately NOT built (documented gaps): per-factor paper-portfolio metrics (needs a hypothetical single-factor-only portfolio, not just `decompose_backtest`'s aggregate contribution), realized transfer coefficient (meaningless until the backtester models a real target-vs-held execution gap -- current single-shot fill assumption makes held≡target). IC time series + t-stats already exist separately (`research/alpha/ic.py`, Phase 2), not yet combined into this scorecard. |
| `capacity.py` — `CapacityPoint` (dataclass), `capacity_analysis(store, start_date, end_date, book_notional_values, **run_backtest_kwargs)` → `list[CapacityPoint]` | DESIGN.md's "at what AUM does modeled impact eat the alpha?" -- a thin sweep re-running the full backtest pipeline (`run_backtest` -> `summarize_backtest` -> `score_backtest`) once per `book_notional` level, no new modeling: `run_backtest` already threads `book_notional` through BOTH the optimizer's ADV-relative position caps (a fixed dollar ADV cap becomes a tighter WEIGHT cap as AUM grows) and the cost model (impact cost grows faster than linear in trade $ size) -- both real capacity effects fall out of existing, already-tested machinery. Expensive (re-runs the full per-date risk-model + QP-solve pipeline per AUM level) -- matches DESIGN.md's own "re-run quarterly," not for casual/automated use. Verified with real output before writing the test assertion (not guessed): at an extreme AUM (10B against a few-million-ADV synthetic universe) both Sharpe and net return get measurably worse than at a small AUM (10M) on the same fixture. |

### engine/ — **built (broker simulator + order gateway + position keeper + risk checker + market data handler + live AlpacaGateway + execution scheduler + kill switch + compliance checker + reconciliation, 82 Catch2 tests, live end-to-end verified)**
...
| `include/position/PositionKeeper.hpp` / `src/position/PositionKeeper.cpp` — `Position`, `PositionKeeper` | average-cost position/P&L tracker built from fills (DESIGN.md Block 5's position keeper); queried directly by `ExecutionScheduler`, still not fed by `OrderGateway.pump()`'s output specifically (a separate wiring gap). `on_fill(symbol, qty, price, is_buy)`: same-direction fill re-averages cost basis; opposite-direction fill closes open qty first (realizing P&L on the closed portion) and flips to a new position on the other side at the fill price if it overshoots past zero. `position(symbol)` on an untraded symbol returns a default flat `Position{}` (qty=0), not a throw -- unlike `OrderGateway`'s unknown-`OrderId` (a caller bug), never having traded a symbol is normal. `all_positions()` (added 2026-08-05) exposes the full book for aggregation -- `ExecutionScheduler`'s drawdown check needs every position's realized P&L + mark-to-market, not one symbol at a time. |
...
| `tests/test_position_keeper.cpp` | 9 Catch2 scenarios: untraded symbol flat, single buy opens long, same-direction fill re-averages cost, partial close realizes P&L, close-past-zero flips side, short position accumulates/closes with correct sign, symbols independent, `all_positions` empty before any fill, `all_positions` reflects every traded symbol |
...
| `src/broker/AlpacaGateway.hpp/.cpp` — `parse_trade_update()`, `AlpacaGateway` | live `IBrokerGateway` backed by Alpaca's real Trading API (DESIGN.md's execution path) -- REST (`libcurl`) for `submit_order`/`cancel_order` against `POST/DELETE .../v2/orders`, the separate `trade_updates` websocket (`wss://.../stream`, its own auth/subscribe/envelope shape, distinct from the market-data feed) for async order status. `client_order_id` = `"<OrderId>-<run_id_>"` (a per-instance timestamp suffix) on submit so incoming events map back without a reverse lookup (`std::stoull` stops at the first non-digit char); a forward map (`OrderId -> Alpaca UUID`) is kept for cancel, which needs Alpaca's own id. `parse_trade_update()` is a free function (pure JSON-to-`OrderEvent` translation, event-type mapping `new/accepted->Ack, fill->Fill, partial_fill->PartialFill, canceled/expired->Cancel, rejected->Reject`, else skipped) -- offline-testable, same live/offline split as `MarketDataHandler`. First genuinely concurrent class in the codebase: the websocket delivers events on its own thread while the caller's thread calls `submit_order`/`cancel_order`/`poll_events`, guarded by one mutex. **Live-verified end-to-end 2026-08-04** (`engine/tools/smoke_alpaca_gateway.cpp`): submit -> Ack event -> cancel -> Cancel event, full round trip confirmed against the real paper account. Two real bugs found + fixed via that live test: `client_order_id` collided across process restarts (Alpaca enforces account-wide uniqueness, not per-process -- fixed with the `run_id_` suffix); Alpaca's paper environment sends `accepted`, not `new`, as the initial ack event (fixed by mapping both to `Ack`, confirmed via live raw-message capture, not assumed from the docs' event list). `fetch_positions()` (added 2026-08-07) -- `GET /v2/positions`, real endpoint/response shape verified against Alpaca's docs before writing (not guessed), returns `broker::BrokerPosition{symbol, qty}` for `engine::ops::reconcile_positions()`. Not part of `IBrokerGateway` -- reconciling against `BrokerSimulator` would be meaningless (no independent record to diverge from). Not unit-tested (live REST call), same posture as `submit_order`/`cancel_order`. |
| `tests/test_alpaca_gateway.cpp` | 9 Catch2 scenarios covering `parse_trade_update()`: fill/partial_fill/new/accepted/canceled/expired/rejected event-type mapping, untracked event type skipped, non-`trade_updates` stream skipped, unrecognized `client_order_id` skipped |
| `include/execution/ExecutionScheduler.hpp` / `src/execution/ExecutionScheduler.cpp` — `TargetPosition`, `ParsedTargetPortfolio`, `parse_target_portfolio()`, `compute_target_orders()`, `compute_book_value()`, `ExecutionScheduler` | wires every prior engine piece together (DESIGN.md's execution scheduler) -- reads `common/target_portfolio.json` (written by `research/portfolio/export_target.py`), converts target notional to shares using `MarketDataHandler`'s live mid price, diffs against `PositionKeeper`'s current position, builds an `Order` per symbol whose delta exceeds a dust threshold (limit price crosses the spread -- ask for buys, bid for sells; v1's single-shot fill-likely default), risk-checks AND compliance-checks each (both must pass), submits the ones via whichever `IBrokerGateway` is injected. Refuses to trade at all if the file's `status` isn't `"optimal"`, OR if the `KillSwitch` is tripped (checked first, before even opening the file). Each call computes `compute_book_value()` (realized + live mark-to-market P&L across `PositionKeeper::all_positions()`) against a running `high_water_mark_`; a drawdown past `max_drawdown_pct` (of `book_notional`, both constructor defaults, uncalibrated placeholders like the rest of this codebase's constants) auto-trips the kill switch -- DESIGN.md's ops-layer requirement, added 2026-08-05. Calls `compliance_checker.reset()` at the start of every `run_once()` -- duplicate-order detection is scoped to one batch, never spans separate calls/days (added 2026-08-06). Order slicing (TWAP/VWAP) and alpha-decay-driven urgency (DESIGN.md's "limit vs market" choice) are deliberately deferred, documented gaps, not silently missing. Almost fully offline-testable (unlike `AlpacaGateway`/`AlpacaMarketDataStream`) -- every dependency (`MarketDataHandler`, `PositionKeeper`, `RiskChecker`, `ComplianceChecker`, `BrokerSimulator`, `KillSwitch`) is already an in-memory fake-able component; only the file read needs a temp-file fixture. |
| `include/ops/KillSwitch.hpp` / `src/ops/KillSwitch.cpp` — `KillSwitch` | DESIGN.md's ops layer: "one command/flag halts all order flow immediately; engine refuses new orders until manually re-armed." `trip(reason)`/`is_tripped()`/`rearm()`/`reason()` -- deliberately no automatic rearm, since auto-clearing a safety gate would defeat its purpose. Consumed by `ExecutionScheduler`, which checks it first in every `run_once()` call. Deferred ops-layer pieces (documented gaps, not built): daily reconciliation vs broker records, alerting infra, model governance/shadow deployment, per-name intraday loss alerts. |
| `include/compliance/ComplianceChecker.hpp` / `src/compliance/ComplianceChecker.cpp` — `ComplianceResult`, `ComplianceChecker` | DESIGN.md's ops layer: "pre-trade compliance checks... beyond fat-finger/risk: restricted-list, max order size, duplicate-order suppression" -- a distinct component from `RiskChecker`, mirroring its `check(order) -> {passed, reason}` shape but for a conceptually different category (compliance/procedural rules vs financial-risk sizing), per DESIGN.md's own wording. Checked in order: restricted-list -> max order size (a share-count cap, distinct from `RiskChecker`'s dollar-notional fat-finger check) -> duplicate-order suppression. Duplicate detection is scoped to one BATCH via `reset()` (called by `ExecutionScheduler` at the start of every `run_once()`), not this object's whole lifetime -- a real design call (Ethan's, 2026-08-06): without a reset boundary, a legitimate trade recurring on a later day would falsely flag as a duplicate of itself. |
| `include/ops/Reconciliation.hpp` / `src/ops/Reconciliation.cpp` — `ReconciliationBreak`, `ReconciliationResult`, `reconcile_positions()` | DESIGN.md's ops layer: "positions + cash vs broker records every day; any break halts trading until explained." Compares `PositionKeeper`'s local book against `AlpacaGateway::fetch_positions()`'s broker-reported positions, checking BOTH directions: a symbol the local book holds that the broker doesn't (or reports a different qty for), and a symbol the broker reports that the local book has never traded. Pure comparison, fully offline-testable -- doesn't itself trip the `KillSwitch`; that's left to whatever calls it (e.g. a daily reconciliation script), same "policy is the caller's job" posture as `compute_book_value()`. Scoped to positions only -- cash reconciliation (DESIGN.md's other half of this bullet) needs a cash-tracking concept this codebase doesn't have yet, `PositionKeeper` only tracks share positions/P&L; a documented gap, not silently dropped. |
| `tests/test_execution_scheduler.cpp` | 17 Catch2 scenarios: `parse_target_portfolio` reads status+positions, no-quote symbol skipped, buy crosses ask, sell crosses bid, dust delta skipped, `run_once` submits passing orders, `run_once` submits nothing on non-optimal status, `run_once` skips a risk-check failure, `compute_book_value` zero for a flat book / counts realized P&L with no quote / adds unrealized mark-to-market, `run_once` no-ops when the kill switch is already tripped (before even opening the file), `run_once` auto-trips the kill switch on a drawdown breach, `run_once` skips a restricted symbol, `run_once` submits only one of two duplicate target-file rows, a later `run_once()` call's matching order is NOT treated as a duplicate of an earlier call (proves `reset()` runs every call) |
| `tests/test_compliance_checker.cpp` | 7 Catch2 scenarios: passes under every rule, restricted symbol rejected, over-max-shares rejected, an identical resubmit in the same batch is a duplicate, a different order for the same symbol is not, `reset()` clears duplicate state for a new batch, restricted-list is checked first (short-circuit order) |
| `tests/test_reconciliation.cpp` | 6 Catch2 scenarios: matching positions clean, a tiny float difference within tolerance still clean, a local-only position is a break, a broker-only position is a break, a quantity mismatch is a break, multiple independent breaks all captured |
| `tests/test_kill_switch.cpp` | 4 Catch2 scenarios: starts untripped, `trip` sets state+reason, `rearm` clears both, a later `trip` overwrites the reason |
| `tools/run_simulation.cpp` | one-off end-to-end demonstration driver (not part of ctest, same posture as `smoke_alpaca_gateway.cpp`): loads a real target-portfolio file + real last-close prices (seeded as synthetic-5bps-spread quotes, no live feed in this offline run), runs `ExecutionScheduler::run_once()` for the real submitted/rejected order set, then a second pass with fresh `RiskChecker`/`ComplianceChecker` instances re-derives per-order pass/fail reasons without perturbing the real compliance-checker's duplicate-detection state. Injects Ack+Fill into `BrokerSimulator` for every submitted order (IDs are sequential from 1, sole caller of `submit_order` in-process) and applies fills to `PositionKeeper` -- explicitly named in `PositionKeeper`'s own docstring as a wiring gap ("not yet fed by `OrderGateway.pump()`'s output"), closed here for this one-off run only, not a permanent wiring. Then runs `reconcile_positions()` (inherently trivial against `BrokerSimulator`, which has no independent position record) and reports kill-switch/book-value state. Dumps one JSON blob to stdout. Run: `engine/build/run_simulation <prices.json> [target_portfolio.json]`. Real run 2026-08-11 against `common/target_portfolio.json` (991-security solve for 2023-06-30, sparse to 101 nonzero positions): 101/101 candidates submitted (0 rejected -- order sizes $1.3K-$340K stayed under the $1M/$2M risk caps), reconciliation clean, kill switch armed, book value -$2,500 (real bid/ask-cross cost on a fresh book's first fills) -- wired into the monitoring dashboard's status rail + working-orders table. |
| `CMakeLists.txt` (root + tests/) | Catch2 pinned via FetchContent (v3.9.1); IXWebSocket v12.0.1 (`USE_TLS=ON`, auto-selects Apple SecureTransport on macOS) + nlohmann/json v3.12.0 also via FetchContent; system `libcurl` via `find_package(CURL REQUIRED)` (no FetchContent needed, already on macOS); `build/` gitignored; `broker_sim` library now also builds `src/broker/AlpacaGateway.cpp` + `src/order/OrderGateway.cpp` + `src/position/PositionKeeper.cpp` + `src/risk/RiskChecker.cpp` + `src/marketdata/MarketDataHandler.cpp` + `src/marketdata/AlpacaMarketDataStream.cpp` + `src/execution/ExecutionScheduler.cpp` + `src/ops/KillSwitch.cpp` + `src/ops/Reconciliation.cpp` + `src/compliance/ComplianceChecker.cpp`; links `nlohmann_json::nlohmann_json` + `CURL::libcurl` (PRIVATE, .cpp-only use) and `ixwebsocket::ixwebsocket` (PUBLIC -- `AlpacaMarketDataStream.hpp`/`AlpacaGateway.hpp` hold an `ix::WebSocket` member by value, so it leaks into the public interface); `run_simulation` tool links `nlohmann_json::nlohmann_json` directly (not inherited from `broker_sim`, which links it PRIVATE) |
...
- `research/attribution/__init__.py` — exports `AttributionResult`, `BookScorecard`, `CapacityPoint`, `capacity_analysis`, `decompose_backtest`, `score_backtest`

### tests/ — **123 passing, 1 skipped (Python)** + **82 passing (C++, `ctest --test-dir engine/build`)**
| file | covers |
|---|---|
...
| `test_decompose.py` | empty steps -> empty result, end-to-end shape on a real store (factor + specific contribution sum to total), non-optimal steps excluded (contribute 0.0/0.0), a date with no matching factor/specific data is 0.0/0.0 not a gap |
| `test_scorecard.py` | Sharpe ratio matches hand computation / `None` under 2 points / `None` at zero variance, max drawdown matches a hand-computed peak-to-trough example, hit rate matches hand computation, gross/net exposure stay aligned with `result`'s arrays when a step is excluded (mid-list, not just the last one), realized beta is `NaN` under insufficient history, realized beta matches a closed-form case (two identical-return securities -> beta=1.0 for both -> portfolio beta == net exposure) |
| `test_capacity.py` | one `CapacityPoint` per `book_notional` value on a real `run_backtest` pipeline, extreme-AUM case shows measurably worse Sharpe + net return than a small-AUM case on the same fixture (verified with real numeric output before writing the assertion) |
| `test_export_target.py` | writes target notional per symbol (`weight * book_notional`), skips zero-weight positions, creates the parent directory if missing, passes a non-optimal `status` through as-is |
...
security master block 4 (CRSP permno/CUSIP hookup) · `infra/` (all Block 5 C++ engine pieces now built AND wired together: broker simulator, order gateway, position keeper, risk checker, market data handler, live AlpacaGateway, execution scheduler, kill switch, compliance checker, reconciliation -- see engine/ section above; `common/` now populated with the target-portfolio file contract. Ops layer: kill switch + drawdown auto-trip (2026-08-05) + restricted-list/max-order-size/duplicate-order suppression (2026-08-06) + position reconciliation (2026-08-07, positions only -- cash reconciliation needs a cash-tracking concept this codebase doesn't have yet) done; still deferred: alerting infra, model governance/shadow deployment, per-name intraday loss alerts. Still open regardless: no scheduling trigger exists to actually invoke `ExecutionScheduler::run_once` once a day -- that's an external cron/launchd concern, not more C++ code. `research/attribution/` no longer empty -- Block 6 v1 (return decomposition + book-level scorecard + capacity analysis) built 2026-08-05/06, see research/attribution/ section above; still unbuilt: per-factor paper-portfolio metrics, realized transfer coefficient, monitoring dashboard -- book-level Sharpe/max-drawdown/hit-rate/exposure/realized-beta, factor-vs-specific attribution, and capacity analysis now exist, IC time series already existed separately (research/alpha/ic.py) and isn't yet combined into one scorecard)
