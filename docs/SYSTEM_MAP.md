...
### engine/ — **built (broker simulator + order gateway + position keeper, 25 Catch2 tests)**
...
| `CMakeLists.txt` (root + tests/) | Catch2 pinned via FetchContent (v3.9.1); `build/` gitignored; `broker_sim` library now also builds `src/order/OrderGateway.cpp` + `src/position/PositionKeeper.cpp` |
...
### tests/ — **106 passing, 1 skipped (Python)** + **25 passing (C++, `ctest --test-dir engine/build`)**
| file | covers |
|---|---|
...
security master block 4 (CRSP permno/CUSIP hookup) · `research/attribution/` · `engine/` remaining Block 5 (C++) pieces (pre-trade risk checks, market data handler, live `AlpacaGateway` — order gateway state machine + position keeper now built; separate from the now-complete research/backtest/) · `common/` · `infra/`
