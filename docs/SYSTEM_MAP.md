...
### engine/ — **built (broker simulator + order gateway + position keeper + risk checker, 31 Catch2 tests)**
...
| `CMakeLists.txt` (root + tests/) | Catch2 pinned via FetchContent (v3.9.1); `build/` gitignored; `broker_sim` library now also builds `src/order/OrderGateway.cpp` + `src/position/PositionKeeper.cpp` + `src/risk/RiskChecker.cpp` |
...
### tests/ — **106 passing, 1 skipped (Python)** + **31 passing (C++, `ctest --test-dir engine/build`)**
| file | covers |
|---|---|
...
security master block 4 (CRSP permno/CUSIP hookup) · `research/attribution/` · `engine/` remaining Block 5 (C++) pieces (market data handler, live `AlpacaGateway` — order gateway state machine + position keeper + pre-trade risk checks now built; separate from the now-complete research/backtest/) · `common/` · `infra/`
