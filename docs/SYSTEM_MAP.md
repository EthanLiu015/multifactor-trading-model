...
### engine/ — **built (broker simulator + order gateway + position keeper + risk checker + market data handler + live AlpacaGateway + execution scheduler, 55 Catch2 tests, live end-to-end verified)**
...
### tests/ — **109 passing, 1 skipped (Python)** + **55 passing (C++, `ctest --test-dir engine/build`)**
| file | covers |
|---|---|
...
security master block 4 (CRSP permno/CUSIP hookup) · `research/attribution/` · `infra/` (all Block 5 C++ engine pieces now built AND wired together: broker simulator, order gateway, position keeper, risk checker, market data handler, live AlpacaGateway, execution scheduler -- see engine/ section above; `common/` now populated with the target-portfolio file contract. Still open: no scheduling trigger exists to actually invoke `ExecutionScheduler::run_once` once a day -- that's an external cron/launchd concern, not more C++ code)
