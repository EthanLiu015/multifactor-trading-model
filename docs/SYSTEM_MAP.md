...
### engine/ — **built (broker simulator + order gateway + position keeper + risk checker + market data handler + live AlpacaGateway + execution scheduler + kill switch, 66 Catch2 tests, live end-to-end verified)**
...
| `include/execution/ExecutionScheduler.hpp` / `src/execution/ExecutionScheduler.cpp` — `TargetPosition`, `ParsedTargetPortfolio`, `parse_target_portfolio()`, `compute_target_orders()`, `compute_book_value()`, `ExecutionScheduler` | wires every prior engine piece together (DESIGN.md's execution scheduler) -- reads `common/target_portfolio.json` (written by `research/portfolio/export_target.py`), converts target notional to shares using `MarketDataHandler`'s live mid price, diffs against `PositionKeeper`'s current position, builds an `Order` per symbol whose delta exceeds a dust threshold (limit price crosses the spread -- ask for buys, bid for sells; v1's single-shot fill-likely default), risk-checks each via `RiskChecker`, submits the ones that pass via whichever `IBrokerGateway` is injected. Refuses to trade at all if the file's `status` isn't `"optimal"`, OR if the `KillSwitch` is tripped (checked first, before even opening the file). Each call computes `compute_book_value()` (realized + live mark-to-market P&L across `PositionKeeper::all_positions()`) against a running `high_water_mark_`; a drawdown past `max_drawdown_pct` (of `book_notional`, both constructor defaults, uncalibrated placeholders like the rest of this codebase's constants) auto-trips the kill switch -- DESIGN.md's ops-layer requirement, added 2026-08-05. Order slicing (TWAP/VWAP) and alpha-decay-driven urgency (DESIGN.md's "limit vs market" choice) are deliberately deferred, documented gaps, not silently missing. Almost fully offline-testable (unlike `AlpacaGateway`/`AlpacaMarketDataStream`) -- every dependency (`MarketDataHandler`, `PositionKeeper`, `RiskChecker`, `BrokerSimulator`, `KillSwitch`) is already an in-memory fake-able component; only the file read needs a temp-file fixture. |
| `include/ops/KillSwitch.hpp` / `src/ops/KillSwitch.cpp` — `KillSwitch` | DESIGN.md's ops layer: "one command/flag halts all order flow immediately; engine refuses new orders until manually re-armed." `trip(reason)`/`is_tripped()`/`rearm()`/`reason()` -- deliberately no automatic rearm, since auto-clearing a safety gate would defeat its purpose. Consumed by `ExecutionScheduler`, which checks it first in every `run_once()` call. Deferred ops-layer pieces (documented gaps, not built): daily reconciliation vs broker records, alerting infra, restricted-list/duplicate-order suppression, model governance/shadow deployment, per-name intraday loss alerts. |
| `tests/test_execution_scheduler.cpp` | 13 Catch2 scenarios: `parse_target_portfolio` reads status+positions, no-quote symbol skipped, buy crosses ask, sell crosses bid, dust delta skipped, `run_once` submits passing orders, `run_once` submits nothing on non-optimal status, `run_once` skips a risk-check failure, `compute_book_value` zero for a flat book / counts realized P&L with no quote / adds unrealized mark-to-market, `run_once` no-ops when the kill switch is already tripped (before even opening the file), `run_once` auto-trips the kill switch on a drawdown breach |
...
### tests/ — **123 passing, 1 skipped (Python)** + **66 passing (C++, `ctest --test-dir engine/build`)**
| file | covers |
|---|---|
...
security master block 4 (CRSP permno/CUSIP hookup) · `infra/` (all Block 5 C++ engine pieces now built AND wired together: broker simulator, order gateway, position keeper, risk checker, market data handler, live AlpacaGateway, execution scheduler, kill switch -- see engine/ section above; `common/` now populated with the target-portfolio file contract. Ops layer v1 (kill switch + drawdown auto-trip) done 2026-08-05; still deferred: daily reconciliation, alerting infra, restricted-list/duplicate-order suppression, model governance/shadow deployment, per-name intraday loss alerts. Still open regardless: no scheduling trigger exists to actually invoke `ExecutionScheduler::run_once` once a day -- that's an external cron/launchd concern, not more C++ code. `research/attribution/` no longer empty -- Block 6 v1 (return decomposition + book-level scorecard + capacity analysis) built 2026-08-05/06, see research/attribution/ section above; still unbuilt: per-factor paper-portfolio metrics, realized transfer coefficient, monitoring dashboard -- book-level Sharpe/max-drawdown/hit-rate/exposure/realized-beta, factor-vs-specific attribution, and capacity analysis now exist, IC time series already existed separately (research/alpha/ic.py) and isn't yet combined into one scorecard)
