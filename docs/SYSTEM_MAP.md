...
### tests/ — **109 passing, 1 skipped (Python)** + **66 passing (C++, `ctest --test-dir engine/build`)**
| file | covers |
|---|---|
...
security master block 4 (CRSP permno/CUSIP hookup) · `research/attribution/` (Block 6 Performance Analysis + analytics suite, DESIGN.md build-phasing items 8-9) · `infra/` (all Block 5 C++ engine pieces now built AND wired together: broker simulator, order gateway, position keeper, risk checker, market data handler, live AlpacaGateway, execution scheduler, kill switch -- see engine/ section above; `common/` now populated with the target-portfolio file contract. Ops layer v1 (kill switch + drawdown auto-trip) done 2026-08-05; still deferred: daily reconciliation, alerting infra, restricted-list/duplicate-order suppression, model governance/shadow deployment, per-name intraday loss alerts. Still open regardless: no scheduling trigger exists to actually invoke `ExecutionScheduler::run_once` once a day -- that's an external cron/launchd concern, not more C++ code)
