...
| Backtester (Block 5) | **blocks 5a+5b of 5d BUILT** | `research/backtest/` (new dir, sibling to signals/alpha/risk/portfolio/attribution). Block 5a: `trade_cost` — research-side cost model (spread + square-root impact + fees; borrow/financing deferred, same free-data gap as block 4b) since Block 5/C++ hasn't built its own cost model yet. Block 5b: `run_backtest`/`BacktestStep` — the walk-forward day-loop (the sole sanctioned Python loop-over-dates in this codebase), chains `build_target_portfolio` across `universe_monthly` rebalance dates threading `w_prev` forward, applies 5a's cost model, computes each step's holding-period return (`Σ w_i × period_return_i`, a documented buy-and-hold-between-rebalances approximation). Widened `TargetPortfolio` (block 4d) with `w_prev`/`adv` fields to avoid re-deriving already-aligned data. 6 tests across the two files. **Next**: 5c (result/equity-curve object), then 5d (held-out-final-years guard) |
...
| Python test suite | **99 passing** — `.venv/bin/python -m pytest` | |
| C++ test suite | **6 passing** — `ctest --test-dir engine/build --output-on-failure` | |

Full Python test suite currently: `99 passed`. Baseline discipline: run it before and after every change set.
...
- Run: `.venv/bin/python -m pytest` (99 passing). Narrow first during debugging (`pytest tests/test_x.py::test_y -x`), full suite as the gate. `test_universe.py`/`test_security_master.py` don't fake a vendor (nothing to fake) but still use a fresh `tmp_path` `PITStore` — no network either way.
...
10. **Block 5: Backtester, scoped 2026-07-23, blocks 5a+5b DONE same day** — `research/backtest/costs.py` (`trade_cost`: spread + sqrt impact + fees, borrow deferred), `research/backtest/simulate.py` (`run_backtest`/`BacktestStep`: walk-forward day-loop, `TargetPortfolio` widened with `w_prev`/`adv`). `.venv/bin/python -m pytest -q` -> "99 passed". **Next**: 5c (result object) -> 5d (held-out guard).
