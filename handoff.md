...
| C++ engine — full Block 5 piece set + execution scheduler + ops layer (broker sim, order gateway, position keeper, risk checker, market data handler, live AlpacaGateway, execution scheduler, kill switch, compliance checker) | **BUILT** | `engine/` (CMake + Catch2 v3.9.1, C++20): `IBrokerGateway`/`Order`/`OrderEvent` interface, `BrokerSimulator` (submit/cancel/poll_events + inject_ack/inject_fill/inject_reject/inject_cancel_ack test-scripting API), `EventJournal` CSV append-log stub, `OrderGateway` (`include/order/`, `src/order/`) — broker-agnostic order state machine wrapping any `IBrokerGateway`, `pump()` drains events into gateway-owned state, illegal transitions on a terminal order throw `std::logic_error`; `PositionKeeper` (`include/position/`, `src/position/`) — average-cost position/P&L tracker built from fills, consumed by `ExecutionScheduler` for current-position lookups and by the ops-layer drawdown check via its `all_positions()` full-book accessor (still not fed by `OrderGateway.pump()`'s output specifically — that wiring gap remains); `RiskChecker` (`include/risk/`, `src/risk/`) — pre-trade fat-finger/position-limit/buying-power checks, composes `PositionKeeper` state, returns a `RiskResult` rather than throwing; `MarketDataHandler` + `AlpacaMarketDataStream` (`include+src/marketdata/`) — top-of-book tracker fed by a live `IXWebSocket` connection to Alpaca's real IEX feed; `AlpacaGateway` (`src/broker/`) — live `IBrokerGateway` backed by Alpaca's real Trading API, REST (system `libcurl`) for submit/cancel + a separate `trade_updates` websocket for async order status, first genuinely concurrent class in the codebase (mutex-guarded shared state between the websocket thread and the caller); `ExecutionScheduler` (`include+src/execution/`) — wires every piece above together: checks the kill switch first, computes book value vs a running high-water mark (auto-trips the kill switch on a drawdown breach), reads `common/target_portfolio.json`, converts target notional to shares via live price, diffs against current position, risk-checks AND compliance-checks (both must pass), submits via whichever `IBrokerGateway` is injected; order slicing and alpha-decay urgency deliberately deferred (documented gaps); `KillSwitch` (`include+src/ops/`) — DESIGN.md's ops layer: `trip`/`is_tripped`/`rearm`, deliberately no automatic rearm; `ComplianceChecker` (`include+src/compliance/`) — DESIGN.md's ops layer: restricted-list, max order size, duplicate-order suppression (a distinct concern from `RiskChecker`'s financial-risk sizing, per DESIGN.md's own wording) — duplicate detection is scoped to one `ExecutionScheduler::run_once()` batch via `reset()`, not this object's whole lifetime, or a legitimate repeated trade on a later day would false-positive; `reconcile_positions()` (`include+src/ops/Reconciliation.{hpp,cpp}`) — DESIGN.md's ops layer: positions vs broker records, checks both directions (local-only, broker-only, quantity mismatch) via `AlpacaGateway::fetch_positions()` (`GET /v2/positions`, real endpoint verified before writing), pure/offline-testable, scoped to positions only (no cash-tracking concept exists yet). All live/network pieces split into offline-testable pure logic + not-unit-tested live glue (deps: IXWebSocket v12.0.1, nlohmann/json v3.12.0 via FetchContent, system libcurl via `find_package`); 82 Catch2 tests. **All Block 5 C++ engine pieces now built AND wired together, plus ops-layer kill switch/drawdown/compliance/reconciliation** — remaining ops-layer pieces (cash reconciliation, alerting, model governance/shadow deployment, per-name intraday alerts) are deliberately deferred, each its own scoping session; separately, no scheduling trigger exists to invoke `ExecutionScheduler::run_once` daily (external cron/launchd concern, not C++ code). Requires Homebrew LLVM (`-DCMAKE_CXX_COMPILER=/opt/homebrew/opt/llvm/bin/clang++`) — system AppleClang/CommandLineTools libc++ headers are broken on the dev machine. `inject_fill` limit-price validation gap **FIXED 2026-07-28**. Market data handler **live-verified 2026-08-02**: connect/auth/subscribe confirmed against the real Alpaca server; no live tick observed yet (verified on a Sunday night, markets closed) — re-run `engine/build/smoke_market_data AAPL 30` during market hours to confirm the tick-parsing path end-to-end. **AlpacaGateway live-verified end-to-end 2026-08-04**: full submit -> Ack -> cancel -> Cancel round trip confirmed against the real paper account (`engine/tools/smoke_alpaca_gateway.cpp`). Two real bugs found + fixed via that test: `client_order_id` collided across process restarts (Alpaca enforces account-wide uniqueness, not per-process); Alpaca's paper environment sends `accepted`, not `new`, as the initial ack event (confirmed via live raw-message capture, both now mapped to `Ack`) |
...
| Ops layer | **BUILT**: kill switch + book-drawdown auto-trip (`engine/include+src/ops/KillSwitch.{hpp,cpp}`) + restricted-list/max-order-size/duplicate-order suppression (`engine/include+src/compliance/ComplianceChecker.{hpp,cpp}`) + position reconciliation (`engine/include+src/ops/Reconciliation.{hpp,cpp}`, positions only — cash reconciliation needs a cash-tracking concept this codebase doesn't have yet) — see C++ engine row above. Alerting infra, model governance/shadow deployment, per-name intraday loss alerts: designed only, each deferred as its own scoping session | |
| Analytics suite / attribution | **v1 BUILT** — `research/attribution/` (DESIGN.md architecture Block 6 — Performance Analysis/attribution, NOT the same "6" as the build-phasing item above; see DESIGN.md's own two numbering systems). `decompose.py`'s `decompose_backtest` decomposes realized backtest returns into factor-bet vs specific contribution, reusing `research/risk/model.py`'s per-date factor/specific returns (widened `build_factor_return_history` to also expose exposure history it already computed internally). `scorecard.py`'s `score_backtest` adds book-level metrics: Sharpe ratio, max drawdown, hit rate, gross/net exposure per date, and realized beta per date (`research/portfolio/beta.py`'s `compute_market_beta` dotted with weights — via a deliberate, flagged loop-over-dates exception, small-N over an already-completed backtest, not a big-data table). `capacity.py`'s `capacity_analysis` sweeps `run_backtest` across `book_notional` levels — no new modeling, `run_backtest` already threads AUM through both the optimizer's ADV caps and the cost model; verified with real numeric output (10M vs 10B AUM, both Sharpe and net return measurably worse at scale) before writing the test. Still designed only: per-factor paper-portfolio metrics, realized transfer coefficient (needs a real target-vs-held execution gap the backtester doesn't model yet), monitoring dashboard. IC time series + t-stats already existed separately (`research/alpha/ic.py`, Phase 2) | |
| Python test suite | **123 passing, 1 skipped** — `.venv/bin/python -m pytest` (the 1 skip is `test_delta_store.py`, collection-skipped outside `.venv-delta/`) | |
| C++ test suite | **82 passing** — `ctest --test-dir engine/build --output-on-failure` | |

Full Python test suite currently: `123 passed, 1 skipped`. Baseline discipline: run it before and after every change set.
...
common/                 target_portfolio.json file contract (README.md documents the schema);
                        Python writer research/portfolio/export_target.py, C++ reader
                        engine/include/execution/ExecutionScheduler.hpp (created 2026-08-04)
infra/                  NOT YET CREATED — reserved by DESIGN.md repo-layout section
...
Planned-but-absent directories (`infra/`, …) are named in DESIGN.md's repo-layout section; do not create them before their block's build phase. `research/attribution/` and `common/` were both in this category — both now created and populated (2026-08-05 and 2026-08-04 respectively).
...
13. **Databricks Free Edition serverless hits `ISOLATION_STARTUP_FAILURE.SANDBOX_STARTUP` on every `applyInPandas` attempt so far** (`exec format error` inside Databricks' own managed UDF-isolation container) — reproduced identically twice (2026-07-28, hours apart, different container IDs each time). Community reports describe this error class as generally intermittent, but no documented workaround was found, and it's a container-*startup* failure (fails before any of our code or data runs), so a smaller-scale attempt wouldn't route around it either. Genuinely external — the error text itself says "Please contact Databricks support." **Block 6d-iv PAUSED** as of 2026-07-28; 6c's live-verified data port (6.28M + 183K rows) stands as the block's resume story regardless of whether this ever resolves. **Root cause identified 2026-07-30** (external research, no code change): matches a named Databricks DBR regression (14.3.11) that broke UDF sandbox isolation on both classic shared-access-mode clusters and serverless, confirmed by Databricks engineering, recurring even past its own scheduled fix date ([community thread](https://community.databricks.com/t5/data-engineering/isolation-startup-failure/td-p/88777), [KB article](https://kb.databricks.com/%5Bisolation_startup_failure%5D-in-shared-clusters-with-databricks-runtime-143-lts-154-lts)). The community-suggested workaround (pin an older DBR build via "Custom Spark Version") needs a classic cluster — Free Edition has none, serverless-only, no custom compute config ([confirmed](https://docs.databricks.com/aws/en/getting-started/free-edition-limitations)). No client-side fix exists; only a support ticket or the platform bug resolving on its own.
