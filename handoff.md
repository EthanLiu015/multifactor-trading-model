...
| Python test suite | **109 passing, 1 skipped** — `.venv/bin/python -m pytest` (the 1 skip is `test_delta_store.py`, collection-skipped outside `.venv-delta/`) | |
| C++ test suite | **66 passing** — `ctest --test-dir engine/build --output-on-failure` | |

Full Python test suite currently: `109 passed, 1 skipped`. Baseline discipline: run it before and after every change set.
