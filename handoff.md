...
| C++ engine — broker simulator + order gateway + position keeper + risk checker + market data handler | **BUILT** | `engine/` (CMake + Catch2 v3.9.1, C++20): `IBrokerGateway`/`Order`/`OrderEvent` interface, `BrokerSimulator` (submit/cancel/poll_events + inject_ack/inject_fill/inject_reject/inject_cancel_ack test-scripting API), `EventJournal` CSV append-log stub, `OrderGateway` (`include/order/`, `src/order/`) — broker-agnostic order state machine wrapping any `IBrokerGateway`, `pump()` drains events into gateway-owned state, illegal transitions on a terminal order throw `std::logic_error`; `PositionKeeper` (`include/position/`, `src/position/`) — average-cost position/P&L tracker built from fills, standalone (not yet wired to `OrderGateway`'s output, waits on the execution scheduler); `RiskChecker` (`include/risk/`, `src/risk/`) — pre-trade fat-finger/position-limit/buying-power checks, composes `PositionKeeper` state, returns a `RiskResult` rather than throwing; `MarketDataHandler` + `AlpacaMarketDataStream` (`include+src/marketdata/`) — top-of-book tracker fed by a live `IXWebSocket` connection to Alpaca's real IEX feed, split into offline-testable parsing logic + not-unit-tested live connection glue (new deps: IXWebSocket v12.0.1, nlohmann/json v3.12.0, both via CMake FetchContent); 38 Catch2 tests. Live AlpacaGateway is the one remaining unscoped piece. Requires Homebrew LLVM (`-DCMAKE_CXX_COMPILER=/opt/homebrew/opt/llvm/bin/clang++`) — system AppleClang/CommandLineTools libc++ headers are broken on the dev machine. `inject_fill` limit-price validation gap **FIXED 2026-07-28** — throws `std::invalid_argument` on a fill that violates the order's non-market `limit_price`. Market data handler **live-verified 2026-08-02**: connect/auth/subscribe confirmed against the real Alpaca server; no live tick observed yet (verified on a Sunday night, markets closed) — re-run `engine/build/smoke_market_data AAPL 30` during market hours to confirm the tick-parsing path end-to-end |
...
| C++ test suite | **38 passing** — `ctest --test-dir engine/build --output-on-failure` | |
