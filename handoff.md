...
| Analytics suite / attribution | **v1 BUILT** — `research/attribution/` (DESIGN.md architecture Block 6 — Performance Analysis/attribution, NOT the same "6" as the build-phasing item above; see DESIGN.md's own two numbering systems). `decompose.py`'s `decompose_backtest` decomposes realized backtest returns into factor-bet vs specific contribution, reusing `research/risk/model.py`'s per-date factor/specific returns (widened `build_factor_return_history` to also expose exposure history it already computed internally). `scorecard.py`'s `score_backtest` adds book-level metrics: Sharpe ratio, max drawdown, hit rate, gross/net exposure per date. Still designed only: per-factor paper-portfolio metrics, realized transfer coefficient (needs a real target-vs-held execution gap the backtester doesn't model yet), realized beta, capacity analysis, monitoring dashboard. IC time series + t-stats already existed separately (`research/alpha/ic.py`, Phase 2) | |
| Python test suite | **119 passing, 1 skipped** — `.venv/bin/python -m pytest` (the 1 skip is `test_delta_store.py`, collection-skipped outside `.venv-delta/`) | |
| C++ test suite | **66 passing** — `ctest --test-dir engine/build --output-on-failure` | |

Full Python test suite currently: `119 passed, 1 skipped`. Baseline discipline: run it before and after every change set.
