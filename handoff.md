...
| Analytics suite / attribution | **v1 BUILT** — `research/attribution/decompose.py`'s `decompose_backtest` (DESIGN.md architecture Block 6 — Performance Analysis/attribution, NOT the same "6" as the build-phasing item above; see DESIGN.md's own two numbering systems). Decomposes realized backtest returns into factor-bet vs specific contribution, reusing `research/risk/model.py`'s per-date factor/specific returns (widened `build_factor_return_history` to also expose exposure history it already computed internally). Per-factor Sharpe/IC/drawdown, realized transfer coefficient, capacity analysis, monitoring dashboard: designed only | |
| Python test suite | **113 passing, 1 skipped** — `.venv/bin/python -m pytest` (the 1 skip is `test_delta_store.py`, collection-skipped outside `.venv-delta/`) | |
| C++ test suite | **66 passing** — `ctest --test-dir engine/build --output-on-failure` | |

Full Python test suite currently: `113 passed, 1 skipped`. Baseline discipline: run it before and after every change set.
