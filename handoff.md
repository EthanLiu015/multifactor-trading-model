...
| C++ engine — broker simulator + order gateway + position keeper + risk checker | **BUILT** | `engine/` (CMake + Catch2 v3.9.1, C++20): `IBrokerGateway`/`Order`/`OrderEvent` interface, `BrokerSimulator` (submit/cancel/poll_events + inject_ack/inject_fill/inject_reject/inject_cancel_ack test-scripting API), `EventJournal` CSV append-log stub, `OrderGateway` (`include/order/`, `src/order/`) — broker-agnostic order state machine wrapping any `IBrokerGateway`, `pump()` drains events into gateway-owned state, illegal transitions on a terminal order throw `std::logic_error`; `PositionKeeper` (`include/position/`, `src/position/`) — average-cost position/P&L tracker built from fills, standalone (not yet wired to `OrderGateway`'s output, waits on the execution scheduler); `RiskChecker` (`include/risk/`, `src/risk/`) — pre-trade fat-finger/position-limit/buying-power checks, first piece composing two components (reads `PositionKeeper` state), returns a `RiskResult` rather than throwing; 31 Catch2 tests. Market data handler and live AlpacaGateway are separate, unscoped. Requires Homebrew LLVM (`-DCMAKE_CXX_COMPILER=/opt/homebrew/opt/llvm/bin/clang++`) — system AppleClang/CommandLineTools libc++ headers are broken on the dev machine. `inject_fill` limit-price validation gap **FIXED 2026-07-28** — throws `std::invalid_argument` on a fill that violates the order's non-market `limit_price` |
...
| C++ test suite | **31 passing** — `ctest --test-dir engine/build --output-on-failure` | |
