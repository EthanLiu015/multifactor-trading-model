...
| C++ engine — full Block 5 piece set (broker sim, order gateway, position keeper, risk checker, market data handler, live AlpacaGateway) | **BUILT** | `engine/` (CMake + Catch2 v3.9.1, C++20): `IBrokerGateway`/`Order`/`OrderEvent` interface, `BrokerSimulator` (submit/cancel/poll_events + inject_ack/inject_fill/inject_reject/inject_cancel_ack test-scripting API), `EventJournal` CSV append-log stub, `OrderGateway` (`include/order/`, `src/order/`) — broker-agnostic order state machine wrapping any `IBrokerGateway`, `pump()` drains events into gateway-owned state, illegal transitions on a terminal order throw `std::logic_error`; `PositionKeeper` (`include/position/`, `src/position/`) — average-cost position/P&L tracker built from fills, standalone (not yet wired to `OrderGateway`'s output, waits on the execution scheduler); `RiskChecker` (`include/risk/`, `src/risk/`) — pre-trade fat-finger/position-limit/buying-power checks, composes `PositionKeeper` state, returns a `RiskResult` rather than throwing; `MarketDataHandler` + `AlpacaMarketDataStream` (`include+src/marketdata/`) — top-of-book tracker fed by a live `IXWebSocket` connection to Alpaca's real IEX feed; `AlpacaGateway` (`src/broker/`) — live `IBrokerGateway` backed by Alpaca's real Trading API, REST (system `libcurl`) for submit/cancel + a separate `trade_updates` websocket for async order status, `client_order_id` = `str(OrderId)` avoids a reverse id-lookup, first genuinely concurrent class in the codebase (mutex-guarded shared state between the websocket thread and the caller). All live/network pieces split into offline-testable pure logic + not-unit-tested live glue (deps: IXWebSocket v12.0.1, nlohmann/json v3.12.0 via FetchContent, system libcurl via `find_package`); 46 Catch2 tests. **All Block 5 C++ engine pieces now built** — nothing left unscoped in this area. Requires Homebrew LLVM (`-DCMAKE_CXX_COMPILER=/opt/homebrew/opt/llvm/bin/clang++`) — system AppleClang/CommandLineTools libc++ headers are broken on the dev machine. `inject_fill` limit-price validation gap **FIXED 2026-07-28**. Market data handler **live-verified 2026-08-02**: connect/auth/subscribe confirmed against the real Alpaca server; no live tick observed yet (verified on a Sunday night, markets closed) — re-run `engine/build/smoke_market_data AAPL 30` during market hours to confirm the tick-parsing path end-to-end. **AlpacaGateway live-verified end-to-end 2026-08-04**: full submit -> Ack -> cancel -> Cancel round trip confirmed against the real paper account (`engine/tools/smoke_alpaca_gateway.cpp`). Two real bugs found + fixed via that test: `client_order_id` collided across process restarts (Alpaca enforces account-wide uniqueness, not per-process); Alpaca's paper environment sends `accepted`, not `new`, as the initial ack event (confirmed via live raw-message capture, both now mapped to `Ack`) |
...
| C++ test suite | **47 passing** — `ctest --test-dir engine/build --output-on-failure` | |
